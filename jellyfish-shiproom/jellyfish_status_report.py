...
def duplicate_template_as_last_week_friday(slides_svc, drive_svc, template_id, parent_folder_id=None):
    ...
    print(f"[COPY] title={new_title}  parent_folder_id={parent_folder_id or '(none)'}")
...
class StatusReportGenerator:
    ...
    def __init__(self, config: Dict[str, Any]):
        # 1) Keep the loaded config dict (stop re-reading YAML here)
        self.config = config or {}

        # 2) Set up your existing clients (keep your wrapper!)
        ...
        # 3) Status mapping for Google Sheets
        self.status_mapping = STATUS_MAPPING

        # 4) Teams config and selection
        ...
        # 5) Google Slides IDs from config.yaml (already loaded)
        ...
        # 6) Build raw Google API services alongside your wrapper
    ...
    def run(self):
        # 1) Make the archive copy named with last week's Friday
        ...
        # 2) Run the report updates AGAINST THE TEMPLATE (not the copy)
        self.generate_slides(self.template_presentation_id)    
